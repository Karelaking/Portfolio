...
export type BoundaryKind = "route-segment" | "explicit-suspense" | "component";
...
export async function countBoundaries(
	page: Page
): Promise<{ total: number; suspended: number }> {
	const boundaries = await page
		.evaluate(inPageSuspense, false)
		.catch(() => [] as Boundary[]);
	const nonRoot = boundaries.filter((b) => b.parentID !== 0);
	return {
		total: nonRoot.length,
		suspended: nonRoot.filter((b) => b.isSuspended).length,
	};
}
...
export function formatReport(report: AnalysisReport): string {
	...
	if (report.holes.length > 0) {
		...
		const holesWithDetail = report.holes.filter(
			(h) =>
				h.renderedBy.length > 0 ||
				h.environments.length > 0 ||
				h.blockers.length > 1 ||
				h.unknownSuspenders
		);
			...
			for (const hole of holesWithDetail) {
				...
				if (hole.blockers.length > 1) {
					...
					for (const blocker of hole.blockers.slice(1)) {
						...
						const owner = blocker.ownerName
							? ` initiated by <${blocker.ownerName}>`
							: "";
						const awaiter = blocker.awaiterName
							? ` awaited in <${blocker.awaiterName}>`
							: "";
						lines.push(
							`      - ${blocker.name}: ${blocker.description || "(no description)"}${env}${owner}${awaiter}`
						);
						if (blocker.ownerFrame) {
							const [fn, file, line] = blocker.ownerFrame;
							lines.push(
								`          owner: ${fn || "(anonymous)"} ${file}:${line}`
							);
						} else if (blocker.awaiterFrame) {
							const [fn, file, line] = blocker.awaiterFrame;
							lines.push(
								`          awaiter: ${fn || "(anonymous)"} ${file}:${line}`
							);
						}
					}
				}
	...
	if (report.statics.length > 0) {
		...
		for (const b of report.statics) {
			...
			const src = b.source
				? ` at ${b.source[0]}:${b.source[1]}:${b.source[2]}`
				: "";
			lines.push(`  ${name}${src}`);
		}
	}
...
function buildBoundaryInsight(
	...
	return {
		...
		actionability: Math.max(
			primaryBlocker?.actionability ?? 0,
			boundaryKind === "route-segment" ? 55 : 0
		),
...
function buildRootCauseGroups(holes: BoundaryInsight[]): RootCauseGroup[] {
	...
	for (const hole of holes) {
		...
		if (existing) {
			...
			existing.count++;
			existing.actionability = Math.max(
...
function compareBoundaryInsights(
	a: BoundaryInsight,
	b: BoundaryInsight
): number {
	return (
...
function compareActionableBlockers(
	a: ActionableBlocker,
	b: ActionableBlocker
): number {
	return (
...
function classifyBlocker(
	...
	if (
		name.includes("cache") ||
		suspender.description.toLowerCase().includes("cache")
	) {
		return "cache";
	}
...
function recommendBoundaryFix(
	...
	if (
		boundaryKind === "route-segment" &&
		primaryBlocker?.kind === "client-hook"
	) {
		return {
	...
	if (
		primaryBlocker?.kind === "request-api" ||
		primaryBlocker?.kind === "server-fetch"
	) {
		return {
...
export function annotateReportWithPageMetadata(
	...
	const sessions = Array.isArray(
		(pageMetadata as { sessions?: unknown[] })?.sessions
	)
		? (
				pageMetadata as {
					sessions: { segments?: { path?: string; type?: string }[] }[];
				}
			).sessions
		: [];
	...
	for (const hole of report.holes) {
		...
		const exact = loadingPaths.find(
			(path) => path.split("/").at(-2) === segmentName
		);
		if (exact) {
			hole.fallbackSource = {
				kind: "loading-tsx",
...
async function inPageSuspense(inspect: boolean): Promise<Boundary[]> {
	...
	function parseInspection(boundary: Boundary, data: any) {
		...
		if (rawSuspenders) {
			for (const entry of rawSuspenders) {
				...
				boundary.suspendedBy.push({
					...
					description: desc,
					duration:
						awaited.end && awaited.start
							? Math.round(awaited.end - awaited.start)
							: 0,
		...
		if (data.unknownSuspenders && data.unknownSuspenders !== 0) {
			...
			boundary.unknownSuspenders =
				reasons[data.unknownSuspenders] ?? "unknown reason";
		}
	...
	function parseStack(raw: any): StackFrame[] | null {
		...
		return raw
			...
			.map(
				(f: any) => [f[0] ?? "", f[1] ?? "", f[2] ?? 0, f[3] ?? 0] as StackFrame
			);
	}
