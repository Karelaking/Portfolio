...
if (cmd === "perf") {
	...
	if (d.lcp) {
		const lcpLabel = d.lcp.element
			? ` (${d.lcp.element}${d.lcp.url ? `: ${d.lcp.url.slice(0, 60)}` : ""})`
			: "";
		lines.push(
			`  LCP               ${String(d.lcp.startTime + "ms").padStart(10)}${lcpLabel}`
		);
	} else {
		lines.push("  LCP                        —");
	}
	...
	if (d.hydration) {
		lines.push(
			`## React Hydration — ${d.hydration.duration}ms (${d.hydration.startTime}ms → ${d.hydration.endTime}ms)`
		);
	} else {
	...
	if (d.phases.length > 0) {
		for (const p of d.phases) {
			lines.push(
				`  ${p.label.padEnd(28)} ${String(p.duration + "ms").padStart(10)}  (${p.startTime} → ${p.endTime})`
			);
		}
		lines.push("");
	}
	if (d.hydratedComponents.length > 0) {
		lines.push(
			`## Hydrated components (${d.hydratedComponents.length} total, sorted by duration)`
		);
		const top = d.hydratedComponents.slice(0, 30);
		for (const c of top) {
			lines.push(
				`  ${c.name.padEnd(40)} ${String(c.duration + "ms").padStart(10)}`
			);
		}
	...
	return new Promise((resolve) => {
		...
		const render = () => {
			...
			for (let i = 0; i < items.length; i++) {
				...
				process.stdout.write(
					i === idx ? `\x1B[36m❯ ${items[i]}\x1B[0m` : `  ${items[i]}`
				);
			}
