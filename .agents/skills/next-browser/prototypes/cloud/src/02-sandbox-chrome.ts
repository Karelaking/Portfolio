...
`;
		...
		await sandbox.writeFiles([
			{
				path: "/vercel/sandbox/my-app/puppet-test.js",
				content: Buffer.from(puppeteerScript),
			},
		]);

		const screenshotResult = await run(sandbox, "node", ["puppet-test.js"], {
		...
		const screenshotData = await sandbox.readFileToBuffer({
		...
		try {
			const logs = await shell(
				sandbox,
				"cat /tmp/next-dev.log 2>/dev/null || echo 'no logs'"
			);
			console.error("\n--- Dev server logs ---");
