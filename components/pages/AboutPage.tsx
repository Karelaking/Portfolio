...
export const AboutPage = async (): Promise<React.ReactElement> => {
	...
	return (
		...
		>
			...
			<GSAPEmptySpaceDecorations section="about" />
				...
				<div className="grid flex-1 grid-cols-1 items-stretch divide-y divide-neutral-200 border-b border-neutral-200 bg-neutral-50/70 lg:grid-cols-12 lg:divide-y-0 lg:divide-x dark:divide-neutral-800 dark:border-neutral-800 dark:bg-neutral-900/60">
					...
					<div className="flex flex-col justify-between p-6 sm:p-10 lg:col-span-7 lg:p-12">
						<div>
							...
							<p className="mt-4 text-neutral-600 font-normal text-base leading-relaxed dark:text-neutral-400">
									...
									{currentFocus.map((item, idx) => (
										...
										>
											<div className="flex items-center gap-4">
												<span className="font-mono font-extrabold text-sm text-neutral-400 tracking-widest group-hover:text-black dark:text-neutral-500 dark:group-hover:text-white">
													[0{idx + 1}]
												</span>
												<span className="font-extrabold text-base text-neutral-900 tracking-tight uppercase dark:text-neutral-100">
													{item.label}
												</span>
											</div>
											<IconArrowRight
												className="shrink-0 text-black transition-transform duration-300 group-hover:translate-x-1 dark:text-white"
												size={16}
											/>
										</div>
									))}
