				...
				<dl className="grid grid-cols-2 md:grid-cols-4">
					{stats.map(({ value, label, delta }) => (
						<div
							...
							key={label}
						>
							<dt className="font-bold text-4xl tracking-tight sm:text-5xl">
								{value}
							</dt>
							<dd className="mt-2 text-muted-foreground text-sm">{label}</dd>
							<Badge className="mt-2" variant="secondary">
								<RiArrowUpLine aria-hidden="true" data-icon="inline-start" />
								{delta}
							</Badge>
						</div>
					))}
