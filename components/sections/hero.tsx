...
const CountUpValue = ({ value }: CountUpValueProps): ReactElement => {
  ...
  useEffect((): (() => void) => {
    ...
    if (reduceMotion) {
      setDisplay(`${formatMetricValue(parsed.value, parsed.decimals)}${parsed.suffix}`);
      return () => undefined;
    }
    ...
    const step = (timestamp: number): void => {
      ...
      setDisplay(`${formatMetricValue(current, parsed.decimals)}${parsed.suffix}`);
      if (progress < 1) {
